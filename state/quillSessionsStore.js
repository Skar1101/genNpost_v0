...
const FILE = path.join(DATA_DIR, 'quill-sessions.json')
...
function createSession({ triggerLabel = '🖱 Plan button', pillars = [], suggestions = [] } = {}) {
  const data = _readAll()
...
function updateDraft(sessionId, draftUid, patch) {
  const data = _readAll()
  const s = data.sessions.find(x => x.id === sessionId)
  if (!s) return null
  const d = s.drafts.find(x => x.uid === draftUid)
  if (!d) return null
...
module.exports = { listSessions, readSession, createSession, appendDraft, updateDraft, deleteSession }
